...
**Statuses** (see Appendix A for backend mapping):

| UI Status | Color | Token | Backend `ProcessingStatus` values |
|---|---|---|---|
| parsing | `blue-500` | Processing | `ingesting`, `chunking` |
| ready | `slate-500` | Awaiting action | `completed` |
| extracting | `amber-500` | Active work (pulse) | `extracting`, `reconciling`, `validating` |
| embedding | `cyan-500` | Post-extraction | `embedding` |
| done | `emerald-500` | Complete | `extracted`, `embedded` |
| error | `red-500` | Failed | `failed`, `partial` |
| pending | `gray-400` | Queued | `pending`, `queued` |

Note: The `embedding` status appears after extraction completes and before `done`. Chat requires embedding to function — show a note "Embedding in progress, chat will be available soon" when status is `embedding`.

**Entity type palette** (consistent across entire app):

The canonical entity type list comes from `ontology/core.yaml` (Layer 1) and `ontology/litrpg.yaml` (Layer 2). The frontend palette covers all types that appear in the graph. Types not listed here use `gray-400` as fallback.

| Type | Color | Layer |
|---|---|---|
| Character | `blue-500` | Core |
| Skill | `violet-500` | Genre (LitRPG) |
| Class | `amber-500` | Genre (LitRPG) |
| Event | `rose-500` | Core |
| Location | `emerald-500` | Core |
| Item | `orange-500` | Core |
| System | `cyan-500` | Genre (LitRPG) |
| Title | `fuchsia-500` | Genre (LitRPG) |
| Level | `lime-500` | Genre (LitRPG) |
| Faction | `teal-500` | Core |
| Arc | `slate-400` | Core |
| Creature | `red-500` | Genre (LitRPG) |
| Concept | `indigo-500` | Core |
| Race | `pink-500` | Genre (LitRPG) |
| Prophecy | `purple-500` | Core |

The current codebase (`LABEL_COLORS` in `lib/utils.ts`) uses a different palette. This must be migrated — see Appendix A.
...
`bookId` is the Neo4j Book node ID (8-char UUID prefix, e.g., `a1b2c3d4`). It is returned in the book list API response and used consistently across all book-scoped routes and API calls.
...
/projects/[slug]/books/[bookId]/reader/[chapterNumber] → Reader at chapter
/projects/[slug]/books/[bookId]/extraction → Extraction dashboard
```

Note: Reader routes use `chapterNumber` (integer, 1-indexed) not `chapterId`, since the existing reader API uses chapter numbers: `GET /reader/books/{book_id}/chapters/{chapter_number}/text`.
...
- **Book selector**: dropdown of extracted books (status `done` or `extracted`/`embedded` only). Changes retrieval context.
  - Default: the most recently extracted book
  - Switching books resets the spoiler guard to full range and starts a new thread
- **Spoiler guard**: dropdown `Ch.1–{max}`. Limits retrieval to chapters in this range.
  - Default: full book (Ch.1–{max chapter})
  - Format: "Spoiler: Ch.1–32"
  - When limit < max: amber badge as visual reminder
  - Persisted per-thread in `chat-store.ts`
...
Books are identified by a **Neo4j string ID** (8-char UUID prefix, e.g., `a1b2c3d4`), generated by `BookRepository.create_book()` as `str(uuid.uuid4())[:8]`. This ID is used in:
- All frontend routes (`/books/[bookId]/...`)
- All API calls (`/books/{book_id}/...`)
- Neo4j queries (`MATCH (b:Book {id: $id})`)

Projects are identified by **slug** (e.g., `the-wandering-inn`), not by their PostgreSQL UUID.
...
- `GET /stream/extraction/{book_id}` — SSE extraction progress (Redis pub/sub, events: `started`, `progress`, `done`, `keepalive`)
- `GET /reader/books/{book_id}/chapters/{chapter_number}/text` — chapter text (title, text, word_count)
- `GET /reader/books/{book_id}/chapters/{chapter_number}/paragraphs` — structured paragraphs (type, char offsets)
- `GET /reader/books/{book_id}/chapters/{chapter_number}/entities` — entity annotations with char offsets (for future reader annotations)

### Endpoints Needing Backend Work

| Endpoint | Need | Priority |
|---|---|---|
| `POST /projects/{slug}/books` | Must call `ingest_file()` after upload to parse chapters + chunks. Currently only stores file on disk. | CRITICAL |
| `GET /projects/{slug}/books/{bookId}/chapters` | Return chapter list with word counts. May wrap existing Neo4j Chapter node queries. | CRITICAL |
| `GET /projects/{slug}/books/{bookId}/parsing/status` | Polling endpoint for parsing progress (chapter count updates) | HIGH |
| `DELETE /projects/{slug}/books/{bookId}` | Delete book + cascade (Neo4j nodes + files) | HIGH |
| `PUT /projects/{slug}` | Update project name/description | MEDIUM |
| `DELETE /projects/{slug}` | Delete project + full cascade | MEDIUM |
| `POST /projects/{slug}/books/{bookId}/extract/cancel` | Cancel running extraction. Requires cooperative cancellation (Redis flag checked between chapters by arq worker). | HIGH |
| `POST /projects/{slug}/books/{bookId}/extract/resume` | Resume failed chapters only | MEDIUM |
| `GET /projects/{slug}/graph/subgraph` | Add `limit` and `sort_by=degree` params for >2000 node cap | MEDIUM |

**Note on Reader API**: The existing `/reader/books/{book_id}/chapters/{chapter_number}/text` endpoint already provides chapter text. The frontend Reader will call this directly (no project-scoped wrapper needed). The chapter list endpoint is the only new endpoint needed for the reader.

**Note on Extraction SSE**: The existing `GET /stream/extraction/{book_id}` endpoint already provides real-time extraction events via Redis pub/sub. The frontend `use-extraction-stream.ts` hook will connect to this directly. Event payload: `{ chapter, total, status, entities_found, chapters_done }`.

**Note on Discovery vs Guided**: The project API response already includes `has_profile: boolean`. The frontend uses this to determine button label: `!has_profile` → "Start discovery extraction", `has_profile` → "Start guided extraction".
...
The frontend uses simplified status labels. The backend `ProcessingStatus` enum (in `backend/app/schemas/book.py`) has 12 values. The mapping:
...
| `embedding` | `embedding` | VoyageAI embedding in progress (post-extraction) |
| `done` | `extracted`, `embedded` | Fully complete. `extracted` = extraction done but embedding pending/done. `embedded` = everything done. |
| `error` | `failed`, `partial` | `partial` = some chapters failed, some succeeded |

Implementation: create a `mapBackendStatus(status: string): UIStatus` function in `lib/utils.ts`.

### Entity Type Color Migration

Current `LABEL_COLORS` in `lib/utils.ts` must be replaced. Old → New mapping:

| Type | Old color (current code) | New color (this spec) |
|---|---|---|
| Character | `indigo` (#6366f1) | `blue-500` (#3b82f6) |
| Skill | `emerald` (#10b981) | `violet-500` (#8b5cf6) |
| Class | `yellow` (#eab308) | `amber-500` (#f59e0b) |
| Event | `red` (#ef4444) | `rose-500` (#f43f5e) |
| Location | `blue` (#3b82f6) | `emerald-500` (#10b981) |
| Item | `violet` (#8b5cf6) | `orange-500` (#f97316) |
| Creature | `orange` (#f97316) | `red-500` (#ef4444) |
| Faction | `teal` (#14b8a6) | `teal-500` (#14b8a6) |
| Concept | `gray` (#6b7280) | `indigo-500` (#6366f1) |

New types added: System (`cyan-500`), Title (`fuchsia-500`), Level (`lime-500`), Arc (`slate-400`), Race (`pink-500`), Prophecy (`purple-500`).

Fallback for unknown types: `gray-400`.

### Entity Badge Navigation

The existing `entity-badge.tsx` links to `/entity/{type}/{name}` which is a dead route. Update to navigate to the Graph Explorer with the entity selected: `/projects/[slug]/graph?entity={entityName}`.

### Charting Library

For the extraction donut chart, use **recharts** (already lightweight, React-native, works with SSR). If not already a dependency, add it. Alternative: custom SVG donut component (~50 lines) to avoid a new dependency.

### Accessibility Notes

Deferred to implementation phase but must be addressed:
- Sidebar: keyboard navigable with arrow keys, `Enter` to expand/navigate
- Graph detail panel: focus trap when open, `Escape` to close
- All interactive elements: visible focus ring (`ring-2 ring-offset-2`)
- Status badges: not color-only — include text labels (the spec already does this with `● done`, `○ pending` etc.)
- Reader: proper heading hierarchy (`h1` for chapter title, semantic paragraphs)
- Chat input: `aria-label`, thread sidebar items are `role="listitem"`
