...
services:
  ...
  redis:
    ...
    mem_limit: 512m
    healthcheck:
      test: ["CMD", "redis-cli", "-a", "${REDIS_PASSWORD:-REDACTED}", "ping"]
      interval: 5s
      timeout: 3s
      retries: 5
  ...
  postgres:
    ...
    environment:
      ...
      POSTGRES_DB: ${POSTGRES_DB:-REDACTED}
    volumes:
      - postgres_data:/var/lib/postgresql/data
    mem_limit: 1g
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${POSTGRES_USER:-REDACTED}"]
      interval: 5s
      timeout: 3s
      retries: 5
  ...
  backend:
    ...
    environment:
      ...
      REDIS_URL: redis://:${REDIS_PASSWORD:-REDACTED}@redis:6379
      POSTGRES_URI: postgresql://${POSTGRES_USER:-REDACTED}:${POSTGRES_PASSWORD:-REDACTED}@postgres:5432/${POSTGRES_DB:-REDACTED}
    depends_on:
      neo4j:
  ...
  worker:
    ...
    mem_limit: 4g
    healthcheck:
      test: ["CMD", "python", "-c", "import redis; r=redis.from_url('redis://:${REDIS_PASSWORD:-REDACTED}@redis:6379'); r.ping()"]
      interval: 30s
      timeout: 5s
      retries: 3
