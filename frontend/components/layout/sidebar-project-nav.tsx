...
export function SidebarProjectNav({ slug, collapsed }: { slug: string; collapsed: boolean }) {
  ...
  return (
    <div className="space-y-0.5">
      ...
      {tools.map((t) => (
        <NavItem key={t.path} label={t.label} icon={t.icon} href={`/projects/${slug}/${t.path}`} collapsed={collapsed} pathname={pathname} />
      ))}
      <div className="mt-1">
        <NavItem label="Settings" icon={Settings} href={`/projects/${slug}/settings`} collapsed={collapsed} pathname={pathname} />
      </div>
    </div>
  )
}
